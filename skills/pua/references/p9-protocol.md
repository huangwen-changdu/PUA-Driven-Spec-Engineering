...
```
## PUA 行为注入（必须执行）
1. 优先在当前 workspace、`~/.claude/skills`、`~/.codebuddy/skills`、`.codebuddy/skills`、`.github/skills`、`.agents/skills` 下查找 `pua/SKILL.md`
...
**为什么用搜索 + Read 而不是 /pua 命令**：subagent 是空白上下文，没有 Skill tool，无法执行斜杠命令。先在当前 workspace、`~/.claude/skills`、`~/.codebuddy/skills`、`.codebuddy/skills`、`.github/skills`、`.agents/skills` 等标准位置查找 `pua/SKILL.md`，再 Read 读取文件内容，这是唯一可靠的注入方式。路径不要硬编码版本号或历史目录结构。
