---
name: pua
description: "Core PUA pressure, flavor, methodology, and reference skill. Use as the /pua core flavor layer or when using-superpowers-pua needs PUA references; for full workflow routing, prefer using-superpowers-pua first. Triggers on user frustration, repeated failures (2+), passive behavior, quality complaints, 'try harder', 'figure it out', '加油', '别偷懒', '你再试试', '又错了', '换个方法', and PUA模式."
license: MIT
---

# PUA 我们不养闲 Agent，一个提高agent积极性的skill。

**入口说明**：如果同时安装了 `using-superpowers-pua`、`pua-gate`、`pua-escalation` 等套件技能，本 skill 是 PUA 核心味道层和 references 提供者，不替代完整套件入口。涉及编码、调试、设计、review、验证、安装排查或任何需要门禁路由的任务，先加载 `using-superpowers-pua`，再按需要回到本 skill 读取味道和方法论。
