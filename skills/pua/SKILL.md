---
name: pua
description: "PUA 方法论智能路由引擎 + 统一压力升级引擎 + 开发过程监控者。按任务类型真实引导 LLM 读取对应方法论文件，在开发过程中实时应用方法论指导。Triggers on user frustration, repeated failures (2+), passive behavior, quality complaints, 'try harder', 'figure it out', '加油', '别偷懒', '你再试试', '又错了', '换个方法', and PUA模式."
license: MIT
---

# PUA 方法论智能路由引擎

**核心定位**：PUA 是方法论智能路由引擎，能够根据任务类型真实引导 LLM 读取对应方法论文件，在开发过程中实时应用方法论指导。

**入口说明**：如果同时安装了 `using-superpowers-pua`、`pua-gate`、`pua-escalation` 等套件技能，本 skill 是 PUA 核心方法论路由引擎，不替代完整套件入口。涉及编码、调试、设计、review、验证、安装排查或任何需要门禁路由的任务，先加载 `using-superpowers-pua`，再按需要回到本 skill 读取方法论文件。
...
**🧭 方法论智能路由（核心功能）**：接到任务后，分析任务类型，**真实引导 LLM 读取对应方法论文件**。在 Sprint Banner 中用 `[方法论路由 🧭]` 标注选择原因。详细路由表见 `references/methodology-router.md`，精简版：
...
**真实引导机制**：
1. 接到任务后，先读取 `references/methodology-router.md` 确定任务类型
2. 根据任务类型，**引导 LLM 读取对应的方法论文件**
3. 在开发过程中，**按照方法论文件的指导执行**
4. 失败时，**引导 LLM 读取其他方法论文件**

**用户手动设置的味道 > 自动路由。** 如果用户在 config 里设了味道，用用户的；如果没设，按上表自动选。

**📦 References 方法论文件（真实引导读取）**

本 skill 的 `references/` 目录里藏着完整的方法论文件——**接到任务后必须真实引导 LLM 读取对应方法论文件**，而不是口头上说用方法论。

**真实引导规则**：
1. 接到任务后，先读取 `references/methodology-router.md` 确定任务类型
2. 根据任务类型，**必须读取对应的方法论文件**
3. 在开发过程中，**必须按照方法论文件的指导执行**
4. 失败时，**必须读取其他方法论文件**

| 触发时机 | 必须读取什么 | 为什么 |
|---------|-------------|--------|
| **任务开始** | `references/methodology-router.md` | 确定任务类型和对应方法论 |
| **确定任务类型后** | `references/methodology-{company}.md` | 获取完整方法论指导 |
| **失败切换时** | `references/methodology-router.md` Phase 2 | 获取失败切换链 |
| **需要深度方法论** | `references/methodology-{company}.md` | 获取完整行为约束和决策框架 |
| **需要显示协议** | `references/display-protocol.md` | 获取 Sprint Banner、进度条、KPI 卡格式 |
| **需要味道文化** | `references/flavors.md` | 获取完整文化 DNA + 黑话词库 |

**加载规则**：
- 本 SKILL.md 内置的精简路由表、味道关键词表、旁白示范**足够日常使用**
- References 是"深水区"——**接到任务后必须真实引导 LLM 读取对应方法论文件**
...
```
按 PUA 方式执行：你不会继承主 agent 的完整上下文，必须在自己的任务内先查事实、别猜；输出真实证据；发现不一致要指出；不得空口完成。如需读取本地 PUA 协议，优先在当前 workspace、~/.claude/skills、~/.codebuddy/skills、.codebuddy/skills、.github/skills、.agents/skills 下查找 pua/SKILL.md；找不到时仍按本段注入规则执行。当前任务是：<任务>
```
...
压力升级由 `pua-escalation` 统一处理。当检测到失败信号时，读取 `skills/pua-escalation/SKILL.md` 获取压力升级指导。
...
**方法论切换**：失败时，`pua-escalation` 会引导读取其他方法论文件，实现方法论智能切换。

## 通用方法论（卡壳时强制执行）

通用方法论由 `pua-escalation` 统一执行。当卡壳时，读取 `skills/pua-escalation/SKILL.md` 获取通用方法论指导。
