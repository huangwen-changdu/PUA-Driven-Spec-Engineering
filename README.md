...
> 一套面向所有开发者和团队的**通用 AI 协作服务流程 / 多平台技能框架**，支持 Claude Code、OpenAI Codex CLI、CodeBuddy 和 GitHub Copilot 四个平台。它不是为当前仓库写代码的项目规范，而是可复制到任意代码库的开发 harness：通过自适应门禁、OpenSpec SDD 渐进确认和 Karpathy 编码准则，让 AI 编码助手更严谨、更可控。
...
一套面向**所有开发者和团队**的 **PUA 驱动规格工程服务流程**——无论你用哪个 AI 编码平台、在哪个代码库，都可以拿来用。当前仓库只是流程模板和技能分发源；真正要服务的是接入它的目标项目。核心思路：
...
| **已有功能迭代** | 同一功能继续增强、修复、回滚或重构 | 3-5 步 | 说"继续迭代 X 功能 / 基于 X vN 更新"，AI 先读取功能档案 |
...
同一个功能多次迭代时，不再只创建彼此孤立的 `openspec/changes/{change-name}/` 文档。流程会维护功能长期档案：

```text
openspec/features/{feature-name}/README.md   # 功能背景、当前版本、版本历史、关键决策、已知约束
openspec/changes/{change-name}/              # 单次变更的 Proposal / Design / Tasks
```
...
- `继续迭代 X 功能，增加 Y`
- `基于 X v2 调整 Y`
- `修复 X 功能的历史问题，不要破坏之前修过的 Y`

AI 会先读取 `Feature Ledger`，确认当前版本、最近变更和历史约束，再创建新的 change；变更完成归档后，再回写功能档案，形成连续版本链。
...
| Skill | 何时用 |
|-------|--------|
| `brainstorming-pua` ⭐ | 新功能 / 模糊需求 / 架构变更；OpenSpec 渐进确认（R2+高成熟度可合并层级，3次确认代替4次） |
...
| `executing-plans-pua` | 单通道按计划执行（执行阶段门禁降级，异常触发） |
...
新功能直接写代码是大多数返工的根源。本 skill 强制先完成核心问题澄清，再走 OpenSpec SDD 渐进确认，做一步确认一步，前一层未确认后一层无法开始：
...
| 层 | 内容 | 产出 |
|----|------|------|
| Core Clarification | 目标、范围、约束、验收标准、影响面；未确认不得生成 Proposal | 用户核心回答/确认 |
| Proposal（可合并 Specs） | 为什么改、改什么、影响谁 + 需求边界/验收标准 | `proposal.md` |
| 路径选择 | A（深度澄清）或 B（快速路径）；R3+ 只提供 A | 用户选择确认 |
| Specs（可并入 Proposal） | 需求边界、验收标准、非目标 | `specs/spec.md`（R2+高成熟度可省略） |
| Design | 方案对比、技术设计、接口定义 | `design.md` |
| Tasks | 实现清单、执行顺序、验证计划 | `tasks.md` |

**核心价值**：不允许一次输出全部文档。每层确认后才推进，防止方向错了全部返工。R2+高成熟度可合并层级减少确认轮次。
...
## OpenSpec 四层渐进确认
变更风险 ≥ R2 时，做一步确认一步：Core Clarification → Proposal → (路径选择 A/B) → Specs → Design → Tasks
Core Clarification 必须先确认目标、范围、约束、验收标准、影响面；未确认不得生成 Proposal。
