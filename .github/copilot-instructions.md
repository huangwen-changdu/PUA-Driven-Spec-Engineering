...
[REQUIRED] 本文件是 GitHub Copilot / VS Code 的项目级入口。`AGENTS.md` 是协作中枢；如两者冲突，以 `AGENTS.md` 为准并同步修正本文件。
...
- 当前文件只服务 GitHub Copilot / VS Code 入口；跨平台入口关系由 `AGENTS.md` 统一维护
...
[REQUIRED] 本文件只写 GitHub Copilot / VS Code 会读取和发现的规则入口；跨平台总表只放在 `AGENTS.md`，避免每个平台入口互相复制、互相污染。
...
[REQUIRED] 当前文件只写 GitHub Copilot / VS Code 会加载的规则；跨入口共识以 `AGENTS.md` 为准。

[REQUIRED] 多平台规则更新时，本文件只同步 GitHub Copilot / VS Code 相关约束；禁止把全平台总表粘到本文件。
...
## 附录：Skill 调用关系（GitHub Copilot 特化）

[NOTE] 详细的 skill 调用关系请参考：
- `skills/skill-call-diagram.md` - 调用关系可视化
- `skills/PUA-FLOW.md` - PUA 流程与技能协作指南

**GitHub Copilot 特化调用规则**：
- [REQUIRED] GitHub Copilot 不支持 `use_skill()` 指令，规则必须内联写入本文件
...
**GitHub Copilot 特化功能**：
- 不支持 `use_skill()` 指令
- 自动加载 `.github/copilot-instructions.md` 项目规则
- 支持 `.github/instructions/*.instructions.md` 文件匹配
- 支持 `.github/prompts/*.prompt.md` 提示词触发
