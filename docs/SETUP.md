...
本文档说明如何在 **Claude Code**、**OpenAI Codex CLI**、**CodeBuddy** 和 **GitHub Copilot** 四个平台上配置和触发这套 PUA-Driven Spec Engineering 技能套件。
...
> **核心区别**：Claude Code 和 CodeBuddy 可以发现 `SKILL.md` 技能文件；Codex 和 GitHub Copilot 仅支持纯 Markdown 指令文件，需将 PUA 核心规则**内联写入**对应配置文件。对 Claude Code / CodeBuddy 来说，"安装 skill"只代表可用；要保证完整套件启动，必须显式触发 `use_skill("using-superpowers-pua")`，或在项目指令中要求每次对话第一动作执行它。
...
> **⚠️ 稳定触发说明**：在 `AGENTS.md` 中只写 `use_skill("using-superpowers-pua")` **没有任何效果**，Codex 不加载 `SKILL.md` 也不执行该指令。必须将完整 PUA 规则**内联写入** `AGENTS.md`，规则才会生效。最小可触发配置：`AGENTS.md` 中必须包含三维门禁规则、路由表和编码准则全文。
...
**批量 TDD**：R0-R2 小粒度任务可用批量模式（批量写测试→实现→验证）。
...
2. **Specs**（`openspec/changes/{name}/specs/spec.md`）→ 用户确认（R2+高成熟度可并入 Proposal）
3. **Design**（`openspec/changes/{name}/design.md`）→ 用户确认
4. **Tasks**（`openspec/changes/{name}/tasks.md`）→ 用户确认

前一层未确认，后一层无法正确生成。R2 + 成熟度 5+ 时，Specs 可并入 Proposal 减少确认轮次。
...
变更风险 ≥ R2 时，**做一步确认一步**：
1. **Proposal**（`openspec/changes/{name}/proposal.md`）→ 用户确认
2. **路径选择**（Proposal 确认后）：A（完整 Specs）或 B（writing-plans 快速路径）→ 用户选择
3. **Specs**（`openspec/changes/{name}/specs/spec.md`）→ 用户确认（仅选 A；R2+高成熟度可并入 Proposal）
4. **Design**（`openspec/changes/{name}/design.md`）→ 用户确认
5. **Tasks**（`openspec/changes/{name}/tasks.md`）→ 用户确认

前一层未确认，后一层无法正确生成。不允许一次输出全部四层文档。R3+ 只提供 A 路径。
R2 + 成熟度 5+ 时，Specs 可并入 Proposal 减少确认轮次。
...
创建 `.codebuddy/rules/pua-default-flow/RULE.mdc`（本仓库已内置完整版，直接使用即可）：

> **⚠️ 稳定触发说明**：仅配置单行 `use_skill("using-superpowers-pua")` **不足以**稳定触发 PUA 流程。RULE.mdc 必须包含完整协作规范内容，而不是只写入口指令。
...
将以下内容写入 `.github/copilot-instructions.md`（VS Code Copilot Chat 自动加载）：
...
设计阶段采用 OpenSpec SDD 渐进确认：Proposal → （路径选择 A/B）→ Specs → Design → Tasks。
Proposal 确认后暂停询问：A（完整 Specs 深度澄清）或 B（直接进 writing-plans 快速路径）。R3+ 只提供 A。
R2 + 成熟度 5+ 时，Specs 可并入 Proposal 减少确认轮次。
...
#### CodeBuddy
项目级 `.codebuddy/` > 用户级 `~/.codebuddy/`；RULE.mdc（alwaysApply: true）> 子代理（按需调用）> Skills（按需加载）

#### GitHub Copilot
`.github/copilot-instructions.md`（项目级，自动加载）；用户级通过 VS Code `github.copilot.chat.codeGeneration.instructions` 设置；`.github/instructions/*.instructions.md`（`applyTo` 匹配时注入）
...
→ brainstorming-pua（Core Clarification → OpenSpec Proposal → Specs）
→ writing-plans-pua
...
#### Q: Copilot Chat 没有应用规范
1. 确认 `.github/copilot-instructions.md` 存在于项目根目录
2. 确认文件内容包含完整规范（不能只有一行 `use_skill` 指令）
3. 重新打开 VS Code Copilot Chat 窗口
4. 在新会话中问"当前应用了哪些指令？"验证
